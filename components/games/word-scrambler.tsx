...
export function WordScrambler() {
  ...
  // Prepare game set on start or mode change
  useEffect(() => {
    if (gameStarted) prepareGameSet()
  }, [gameStarted, gameMode])

  // Update current word & display each question
  useEffect(() => {
  ...
  const prepareGameSet = () => {
    ...
    if (gameMode === "missing") {
      setCurrentSet(randomized.map(item => ({
        ...item,
  ...
  // Submit answer handler (now async for leaderboard call)
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (userGuess.toUpperCase() === currentWord) {
      ...
      // ⭐ Send XP to leaderboard
      try {
        await fetch("/api/users/leaderboard/update", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ xpGained: 10, source: "word_scrambler" }),
        })
      } catch (error) {
        console.error("Error updating leaderboard for WordScrambler:", error)
      }
    } else {
  ...
  // Next question / round / end-game logic
  const handleNext = () => {
    ...
    if (nextIndex % questionsPerRound === 0 && nextIndex < totalQuestions) {
      ...
      toast({
        title: "Game Complete!",
        description: `You've completed all rounds with ${score} correct answers.`,
      })
    } else {
      moveToNextQuestion(nextIndex)
    }
  ...
  const resetXP = () => {
    ...
    toast({ title: "XP Reset", description: "XP has been reset to 0" })
  }
  const resetTotalXP = () => {
    setScrambledXP(0)
    setMissingXP(0)
    ...
    toast({ title: "All XP Reset", description: "All XP has been reset to 0" })
  }

  const totalXP = scrambledXP + missingXP

  // Render the dynamic game interface
          ...
          <div className="bg-[#1f2937] p-4 rounded-lg mb-6">
            <div className="flex items-start">
              ...
              <p className="text-gray-300">
                {currentSet[questionIndex]?.meaning}
              </p>
            </div>
          </div>
        )}
  ...
  return (
    <GameWrapper
      ...
      description={`Test your vocabulary skills with ${
        gameMode === "scrambled" ? "Scrambled Words" : "Missing Letters"
      }!`}
    ...
    >
      <Tabs
        ...
        onValueChange={value =>
          handleModeChange(value as "scrambled" | "missing")
        }
      >
        <TabsList className="grid w-full grid-cols-2 bg-[#4cc9f0]/20">
          <TabsTrigger
            value="scrambled"
            className={`${
              gameMode === "scrambled" ? "bg-[#4cc9f0] text-black" : "text-white"
            } font-medium`}
          >
            Scrambled
          </TabsTrigger>
          <TabsTrigger
            value="missing"
            className={`${
              gameMode === "missing" ? "bg-[#4cc9f0] text-black" : "text-white"
            } font-medium`}
          >
            Missing Letters
          </TabsTrigger>
      ...
      <div className="flex flex-wrap justify-between items-center mb-4 gap-2">
        ...
        <Badge className="bg-[#2a3343] px-3 py-1 text-sm font-medium">
          Question: {questionIndex % questionsPerRound + 1}/{questionsPerRound}
        </Badge>
        <Badge className="bg-[#2a3343] px-3 py-1 text-sm font-medium">
          Score: {score}
        </Badge>
        <Badge className="bg-[#2a3343] px-3 py-1 text-sm font-medium">
          {gameMode === "scrambled" ? "Scrambled" : "Missing"} XP:{" "}
          {gameMode === "scrambled" ? scrambledXP : missingXP}
        </Badge>
        <Badge className="bg-[#2a3343] px-3 py-1 text-sm font-medium">
          Total XP: {totalXP}
        </Badge>
      ...
      <Card className="bg-[#2a3343] border-[#3a4353]">
        <CardContent className="pt-6">{renderGameContent()}</CardContent>
      </Card>
    </GameWrapper>
  )
}
