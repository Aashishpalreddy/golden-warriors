...
export function QuizGame() {
  ...
  const totalQuestionsPerRound = rounds.map(r => r.questionsCount)
  const totalQuestions = totalQuestionsPerRound.reduce((sum, cnt) => sum + cnt, 0)

  const handleStartGame = () => {
    // Initialize and start the quiz
    const shuffled = [...quizQuestions].sort(() => 0.5 - Math.random())
    setRoundQuestions(shuffled.slice(0, rounds[0].questionsCount))
    setGameStarted(true)
    setGameOver(false)
  ...
  const handleNextRound = async () => {
    ...
    if (next < rounds.length) {
      // setup next round
      const start = rounds.slice(0, next).reduce((sum, r) => sum + r.questionsCount, 0)
      setRoundQuestions(quizQuestions.slice(start, start + rounds[next].questionsCount))
      ...
      try {
        await fetch("/api/users/leaderboard/update", {
          ...
          body: JSON.stringify({ xpGained: earnedXP, source: "habit_quiz" }),
        })
      } catch (err) {
  ...
  const renderRoundComplete = () => {
    const rs = roundScores[currentRound]
    const total = rounds[currentRound].questionsCount
    const pct = Math.round((rs / total) * 100)
    let msg = pct >= 80
    ...
    return (
      <Card className="bg-[#2a3343] border-[#3a4353]">
        <CardContent className="pt-6 pb-6 text-center">
          ...
          <p className="text-white mb-6">{msg}</p>
          <Button
            className="mt-4 bg-[#4cc9f0] hover:bg-[#4cc9f0]/80 text-black w-full"
            onClick={handleNextRound}
          >
            {currentRound < rounds.length - 1 ? "Start Next Round" : "See Final Results"}
          </Button>
        </CardContent>
      </Card>
    )
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {rounds.map((r, idx) => (
          <div
            key={idx}
            className="flex justify-between items-center p-2 bg-[#2a3343] rounded-md"
          >
            <span>Round {idx + 1}: {r.name}</span>
            <Badge className={roundScores[idx] > r.questionsCount / 2 ? "bg-green-500" : "bg-red-500"}>
              {roundScores[idx]}/{r.questionsCount}
            </Badge>
          </div>
        ))}
      </div>
      <div className="pt-4 border-t border-[#3a4353] flex justify-between items-center">
        <span className="text-lg font-medium">Total Score:</span>
        <Badge className="text-lg bg-[#4cc9f0] text-black">{score}/{totalQuestions}</Badge>
      </div>
    </div>
  )
            ...
            <Card className="bg-[#2a3343] border-[#3a4353]">
              <CardContent className="pt-6">
                ...
                <RadioGroup value={selectedAnswer} className="space-y-3">
                  {roundQuestions[currentQuestion]?.options.map((opt, i) => {
                    let labelCls = "text-white"
                    if (isAnswerChecked) {
                      if (opt === roundQuestions[currentQuestion].correctAnswer) {
                        labelCls = "text-green-400 font-bold"
                      } else if (opt === selectedAnswer && opt !== roundQuestions[currentQuestion].correctAnswer) {
                        labelCls = "text-red-400 line-through"
                      }
                    }
                    return (
                      <div key={i} className="flex items-center space-x-2">
                        <RadioGroupItem
                          value={opt}
                          id={`opt-${i}`}
                          className={opt === roundQuestions[currentQuestion].correctAnswer && isAnswerChecked
                            ? "text-green-400"
                            : "text-[#4cc9f0]"
                          }
                        ...
                        />
                        <Label htmlFor={`opt-${i}`} className={labelCls}>{opt}</Label>
                      </div>
                    )
                  })}
                </RadioGroup>
